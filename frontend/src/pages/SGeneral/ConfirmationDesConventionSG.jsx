...
export default function ConfirmationDesConventionSG() {
    ...
    const handleConfirm = (convention) => {
        MySwal.fire({
            ...
            if (result.isConfirmed) {
                const updatedConvention = { ...convention, is_signer: 1 };
                axios
                    .put(`${appConfig.baseurlAPI}/conventions/${convention.id}`, updatedConvention)
                    .then((response) => {
                        setRows((prevRows) =>
                            prevRows.map((row) =>
                                row.id === convention.id ? { ...row, is_signer: 1 } : row
                            )
                        );
                        Swal.fire(
                            'Confirmé!',
            ...
            .then((response) => {
                if (response.data) {
                    const filteredData = response.data.filter(
                        (convention) => convention.is_telecharger === 1 && convention.is_valider === 1 && convention.is_signer === 0
                    );
